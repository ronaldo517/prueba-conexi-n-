...
app.post('/api/detalle_venta', async (req, res) => {
    const { Cod_venta, Cod_prod, Cant, ValorUnit, Estado } = req.body;

    if (!Cod_venta || !Cod_prod || Cant === undefined || ValorUnit === undefined || Estado === undefined) {
        return res.status(400).json({ error: 'Todos los campos son requeridos' });
    }

    try {
        await db.query(
            "INSERT INTO detalle_venta (Cod_venta, Cod_prod, Cant, ValorUni, Estado) VALUES (?, ?, ?, ?, ?)",
            [Cod_venta, Cod_prod, Cant, ValorUnit, Estado]
        );
...
app.put('/api/detalle_venta/:Cod_ven/:Cod_prod', async (req, res) => {
    const { Cod_venta, Cod_prod } = req.params;
    ...
    try {
        const [result] = await db.query(
            ...
            [Cant, ValorUnit, Estado, Cod_venta, Cod_prod]
        );
...
app.delete('/api/detalle_venta/:Cod_venta/:Cod_prod', async (req, res) => {
    const { Cod_venta, Cod_prod } = req.params;
